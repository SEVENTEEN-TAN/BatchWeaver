...
public class ExecutionContextLogger implements JobExecutionListener, StepExecutionListener {
    ...
    public void afterJob(JobExecution jobExecution) {
        ...
        boolean hasWarnings = stepSummaries.stream().anyMatch(s ->
        ...
        summary.append(String.format("  | %-15s | %-12s | %6s | %6s | %4s | %8s | %-15s |%n",
            "Step Name", "Status", "Read", "Write", "Skip", "Duration", "Note"));
        summary.append("  " + SEPARATOR + "\n");

        for (StepSummary step : stepSummaries) {
            String note = "";
            ...
            summary.append(String.format("  | %-15s | %-12s | %6d | %6d | %4d | %8s | %-15s |%n",
                step.stepName(),
                step.exitStatus(),
                step.readCount(),
                step.writeCount(),
