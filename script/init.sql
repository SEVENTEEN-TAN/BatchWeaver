...
-- Format: YYYYMMDD0001 (12 digits: 8-digit date + 4-digit sequence)
-- Example: 202512310001, 202512310002, ...
DECLARE @datePrefix BIGINT = CAST(FORMAT(GETDATE(), 'yyyyMMdd') AS BIGINT) * 10000;

-- Job Instance ID Sequence (starts from YYYYMMDD0001)
EXEC('CREATE SEQUENCE BATCH_JOB_SEQ START WITH ' + CAST(@datePrefix + 1 AS VARCHAR) + ' INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807');

-- Job Execution ID Sequence (starts from YYYYMMDD0001)
EXEC('CREATE SEQUENCE BATCH_JOB_EXECUTION_SEQ START WITH ' + CAST(@datePrefix + 1 AS VARCHAR) + ' INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807');

-- Step Execution ID Sequence (starts from YYYYMMDD0001)
EXEC('CREATE SEQUENCE BATCH_STEP_EXECUTION_SEQ START WITH ' + CAST(@datePrefix + 1 AS VARCHAR) + ' INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807');
GO
