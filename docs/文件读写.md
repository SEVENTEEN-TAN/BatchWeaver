...
```mermaid
flowchart TB
    subgraph Traditional["传统预扫描方案"]
        O1["Step 1: open()<br/>预扫描整个文件<br/>识别 Header/Footer"]
        O2["❌ 启动延迟高<br/>大文件需要数秒甚至数分钟"]

        O3["Step 2: read()<br/>从 Header 后开始读取"]
        O4["❌ 内存占用大<br/>缓存整个文件或大量行"]

        P1["性能问题:<br/>启动延迟: O(n)<br/>内存占用: O(n)<br/>I/O 次数: 2次"]
...
```mermaid
flowchart TB
    subgraph SinglePass["单次扫描 + 延迟决策方案"]
        S1["Step 1: open()<br/>仅初始化资源<br/>不扫描任何内容"]
        A1["✅ 零启动延迟"]

        S1 --> S2["Step 2: read() - 单次顺序扫描"]

        subgraph ReadProcess["read() 处理流程"]
            R1["读取第一行<br/>→ 延迟判断是否为 Header"]
            R2["读取中间行<br/>→ 作为数据处理"]
            R3["读取最后一行<br/>→ 延迟判断是否为 Footer"]
            H1["解析Header<br/>继续读取数据行"]
            F1["解析Footer<br/>返回null"]
            D1["处理数据"]
        ...
        P1["性能优势:<br/>启动延迟: O(1)<br/>内存占用: O(1)<br/>I/O 次数: 1次"]
    end
...
stateDiagram-v2
    ...
    START --> FIRST_LINE: read()

    FIRST_LINE --> DATA_STATE: 不是 Header
    FIRST_LINE --> DATA_STATE: 是 Header<br/>解析并校验后跳过

    DATA_STATE --> DATA_STATE: read()<br/>检测 Footer(预判)
    DATA_STATE --> FOOTER_CHECK: EOF<br/>(currentLine == null)

    FOOTER_CHECK --> [*]: Footer<br/>解析Footer → 校验 → 返回null
    FOOTER_CHECK --> [*]: 数据行<br/>处理 → actualRecordCount++ → 返回数据
    FOOTER_CHECK --> [*]: 空/null<br/>直接返回 null

    note right of FIRST_LINE
        延迟判断第一行是否为 Header
    end note
