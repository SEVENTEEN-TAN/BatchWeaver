...
| 数据源 | 用途 | 数据库 | 事务管理器 |
|--------|------|--------|-----------|
| **db1** | Spring Batch 元数据 + 业务数据 | BatchWeaverDB | tm1 |
...
flowchart TB
    subgraph Isolation["事务隔离设计铁律"]
        TM1["🔴 元数据事务（tm1）<br/>绝对不受业务事务影响<br/>必须提交成功！"]
        TM2["✅ 业务事务（tm2/tm3/tm4）<br/>管理业务数据操作<br/>失败时可以回滚"]
...
flowchart TB
    subgraph Execution["Step 执行流程"]
        START["开始执行 Step"]
        META_WRITE["[tm1 事务] 元数据写入<br/>JOB_EXECUTION 状态 = STARTED"]
        ...
        TM1_COMMIT["✅ [tm1 事务] 元数据事务提交<br/>→ JOB_EXECUTION 状态 = FAILED"]
...
**关键点**：
- ✅ `@Primary` 注解标记为主数据源
- ✅ Bean 名称使用 `dataSource1`, `jdbcTemplate1`, `tm1`
- ✅ 事务管理器命名为 `tm1`，用于元数据事务
...
- [ ] `DataSource1Config` 标记了 `@Primary`
- [ ] `BatchInfrastructureConfig` 的 JobRepository 绑定了 tm1
