...
| Job名称 | 文件格式 | Header | Footer | 说明 |
|---------|----------|--------|--------|------|
| **masterImportJob** | 串行执行所有格式 | - | - | 依次执行 format1→format2→format3 |
| format1ImportJob | yyyyMMdd + 纯数字 | `20260120` | `3` | 单独执行 Format1 |
| format2ImportJob | MMddyyyy + R前缀 | `01202026` | `R00003` | 单独执行 Format2 |
| format3ImportJob | 无头尾 | 无 | 无 | 单独执行 Format3 |

#### 测试场景

##### 场景1: Master Job 串行导入（推荐）
...
**预期结果**
- ✅ Job状态: COMPLETED
- ✅ format1ImportStep: COMPLETED (读取3条)
- ✅ format2ImportStep: COMPLETED (读取3条)
- ✅ format3ImportStep: COMPLETED (读取3条)
- ✅ DB2总共插入9条记录
...
```mermaid
flowchart LR
    Start[开始] --> S1[format1ImportStep]
    S1 --> S2[format2ImportStep]
    S2 --> S3[format3ImportStep]
    S3 --> End[完成]

    S1 -.失败.-> Fail[Job FAILED]
    S2 -.失败.-> Fail
    S3 -.失败.-> Fail
```
...
##### 场景5: Master Job 中间步骤失败（负向）

**前置条件**
```
data/input/format1_users.txt 正常
data/input/format2_users.txt Footer count=5（实际只有3条数据）
...
- ⏭️ format3ImportStep: SKIPPED (未执行)
- ❌ Job状态: FAILED
- ✅ DB2只插入3条记录（format1的数据）
...
- ⚠️ 日志: "[Format1] Header date mismatch"
