...
- **文档规范**：本文件第 3~5 章中的模式说明、规则矩阵、状态图和错误文案，作为业务与操作层的统一说明；
- **运行时校验**：`ExecutionModeValidator` 作为唯一入口，根据 `_mode`、`id`、`_target_steps` 等参数执行语义校验并抛出异常。

`@BatchJob` 注解仅用于声明 Job 的元数据（步骤列表、是否为条件流），为校验器和动态构建器提供必要信息，不额外承载执行模式语义或控制逻辑。
